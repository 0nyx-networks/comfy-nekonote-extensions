...
class ResizeImage(comfy_api_io.ComfyNode):
    ...
    def execute(cls, image: torch.Tensor, width: int, height: int, **kwargs) -> None:
        try:
            img = Utils.tensor_to_image(image)
            ...
            print(f"[ResizeImage] ERROR: {ex}")
            import traceback
            traceback.print_exc()

        return Utils.create_fallback_image(width, height)
